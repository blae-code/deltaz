...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (plan.status !== 'accepted') {
        return Response.json({ error: `MissionPlan cannot be activated. Status is '${plan.status}'.` }, { status: 409 });
    }
    ...
    console.error('activateMissionPlan error:', error);
    return Response.json({ error: error.message || 'Failed to activate mission plan' }, { status: 500 });
  }
});
