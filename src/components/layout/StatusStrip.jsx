    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
      {items.map((item, i) => (
        <div key={i} className="panel-frame clip-corner-tr px-3 py-2.5 sm:p-3 relative">
          <div className="text-[9px] text-muted-foreground/70 tracking-[0.2em] uppercase font-mono leading-tight mb-1">
            {item.label}
          </div>
          ...
          {item.sub && (
            <div className="text-[9px] text-muted-foreground/60 font-mono mt-1">{item.sub}</div>
          )}
        </div>
      ))}
    </div>
